...
app.get("/", async (req, res) => {        // Cria endpoint na rota da raiz do projeto
  ...
  console.log("Rota GET / solicitada");
   res.json({
		message: "API para Cruzeiro 6 x 1 Atletico-MG",      // Substitua pelo conteúdo da sua API
    author: "Matheus Ruhan Costa",    // Substitua pelo seu nome
...
app.post("/usuarios", async (req, res) => {
  try {
    ...
    const db = conectarBD();
    const consulta = `INSERT INTO usuarios (nome, login, senha, email) VALUES ($1, $2, $3, $4) RETURNING *`;
    const resultado = await db.query(consulta, [nome, login, senha, email]);
...
app.put("/usuarios/:id", async (req, res) => {
  try {
    ...
    if (existente.rows.length === 0) return res.status(404).json({ mensagem: "Usuário não encontrado" });
    ...
    const consulta = `
      UPDATE usuarios SET nome = $1, login = $2, senha = $3, email = $4
      WHERE id = $5 RETURNING *`;
    const resultado = await db.query(consulta, [novoNome, novoLogin, novaSenha, novoEmail, id]);
